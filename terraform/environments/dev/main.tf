provider "aws" {
  ...
  default_tags { tags = { ManagedBy = "terraform", Project = var.project, Environment = "dev", Region = var.aws_region, InfraVersion = var.infra_version } }
}
...
locals {
  ...
  tags = { Name = local.name, Environment = "dev", Region = var.aws_region, InfraVersion = var.infra_version, SecurityContact = "platform@example.com" }
}
